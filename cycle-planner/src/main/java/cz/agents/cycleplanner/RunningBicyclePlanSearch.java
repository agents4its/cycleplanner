...
import cz.agents.cycleplanner.aStar.aima.GraphFunctionFactory;
...
import cz.agents.cycleplanner.nearestNode.NearestNodeJavaApprox;
import cz.agents.cycleplanner.nearestNode.NearestNodeService;
import cz.agents.cycleplanner.routingService.GraphProvider;
...
public class RunningBicyclePlanSearch {
	...
	public void setUp() throws Exception {
		try {
			...
			File g = ResourceToFile.getFileFromResource(CyclewayGraphLoader.getCyclewayGraphStream("prague"));
	...
	private void runTest(Profile profile) throws Exception {
		...
		while (i <= ITERATION) {
			...
			Graph<CycleNode, CycleEdge> graph = addAdditionalElementsToExistingGraph(additionalGraphElements);
	...
	private Graph<CycleNode, CycleEdge> addAdditionalElementsToExistingGraph(AdditionalGraphElements elements) {
		GraphBuilder<CycleNode, CycleEdge> builder = new GraphBuilder<>();
		
		builder.addNodes(cycleGraph.getAllNodes());
		builder.addNodes(elements.getAdditionalNodes());
		
		builder.addEdges(cycleGraph.getAllEdges());
		builder.addEdges(elements.getAdditionalEdges());
