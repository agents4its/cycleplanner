...
package cz.agents.cycleplanner.plannerDataImporterExtensions;
 ...
 * @param <TEdge>
 */
public abstract class RelationalGraphImportTask<TNode extends eu.superhub.wp5.graphcommon.graph.elements.Node, TEdge extends eu.superhub.wp5.graphcommon.graph.elements.Edge>
	...
	private static final Logger logger = Logger
			.getLogger(RelationalGraphImportTask.class);
			...
			for (RelationMember relationMember : relation.getMembers()) {
				if (relationMember.getMemberType() == EntityType.Way) {
					...
					if (way != null) {
						...
						// logger.warn("Relation includes way with id " +
		...
		while (ways.hasNext()) {
			...
			for (WayNode wayNode : way.getWayNodes()) {
				...
				if (node != null) {
					...
					logger.warn("Way includes node with id "
							+ wayNode.getNodeId()
							+ ", which has not representation in OSM data");
				}
			}

			// why not add here? (* question) true represents what?
			// @todo
			...
			if (true) {
				...
				setEdgesToBuilder(builder,
						createEdgesFromOsm(reverseNodes, way.getTags()));
			}
			// vezme všetky nody v danej ceste a setne ich do buildera aj s ich
			// tagmi
	...
	// create edges from OSM ways data (*)
	public List<TEdge> createEdgesFromOsm(ArrayList<Node> nodes,
		...
		for (int i = 0; i < nodes.size() - 1; i++) {
			...
			RelationalGraphImportTask.TupletEdgeId tupletEdgeId = new RelationalGraphImportTask.TupletEdgeId(
					fromNodeOsm.getId(), toNodeOsm.getId());
			...
			if (edge == null) {
				...
				TNode formNode = getFromCacheOrCreateNew(fromNodeOsm,
						fromNodeOsm.getTags());
				TNode toNode = getFromCacheOrCreateNew(toNodeOsm,
						toNodeOsm.getTags());

				edge = createEdge(formNode, toNode, comulativeLength, info);
