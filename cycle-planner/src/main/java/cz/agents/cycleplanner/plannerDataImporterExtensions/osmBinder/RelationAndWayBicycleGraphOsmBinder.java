...
import cz.agents.cycleplanner.plannerDataImporterExtensions.RelationAndWaySelector;
...
public class RelationAndWayBicycleGraphOsmBinder {
	...
	static {
		...
		// this is the same as that defined in Marcel's paper (* question)

		includeRelations.add(TagConditionsEvaluator.createConditionTag("route",
				"bicycle"));

		AtLeastOneIncludedTagSatisfiedEvaluator evaluatorRelations = new AtLeastOneIncludedTagSatisfiedEvaluator(
				includeRelations, excludeRelations);

		Set<String> includeWays = new HashSet<>();
		Set<String> excludeWays = new HashSet<>();

		includeWays.add(TagConditionsEvaluator.createConditionTag("highway",
				"primary"));
		includeWays.add(TagConditionsEvaluator.createConditionTag("highway",
				"primary_link"));
		includeWays.add(TagConditionsEvaluator.createConditionTag("highway",
		...
		excludeWays.add(TagConditionsEvaluator.createConditionTag("bicycle",
				"no"));
		excludeWays.add(TagConditionsEvaluator.createConditionTag("access",
				"customers"));
		excludeWays.add(TagConditionsEvaluator.createConditionTag("access",
				"delivery"));
		excludeWays.add(TagConditionsEvaluator.createConditionTag("access",
		...
		excludeWays.add(TagConditionsEvaluator.createConditionTag("access",
				"no"));
		AtLeastOneIncludedTagSatisfiedEvaluator evaluatorWays = new AtLeastOneIncludedTagSatisfiedEvaluator(
				includeWays, excludeWays);
		...
		permittedModeEvaluator = new PermittedModeEvaluator(modes,
				evaluatorWays);
	}
