...
import cvut.fel.nemetma1.plannerDataImporterExtensions.RelationAndWayOsmSink;
...
public abstract class WayAndRelationalGraphImportTask<TNode extends eu.superhub.wp5.graphcommon.graph.elements.Node, TEdge extends eu.superhub.wp5.graphcommon.graph.elements.Edge>
    ...
    private static final Pattern REVERSE_ONEWAY_VALUE_PATTERN = Pattern.compile("-1|reverse");
    private static final Pattern NOT_ONEWAY_FOR_CYCLEWAY_KEY_PATTERN = Pattern.compile("cycleway|cycleway_left|cycleway_right");
    private static final Pattern NOT_ONEWAY_FOR_CYCLEWAY_VALUE_PATTERN = Pattern.compile("opposite|opposite_lane|opposite_track");
    private static final Pattern NOT_ONEWAY_FOR_BICYCLE_KEY_PATTERN = Pattern.compile("oneway:bicycle");
    private static final Pattern NOT_ONEWAY_FOR_BICYCLE_VALUE_PATTERN = Pattern.compile("no");
     ...
     *
     * @param relations relations that should be included into graph
     * @param ways ways that should be included into graph
     * @param dataset OSM dataset
     * @return graph containing edges and nodes from specified ways and relations
     */
    @Override
    public Graph sink(Iterator<Relation> relations, Iterator<Way> ways, MemoryDataSet dataset) {
        ...
        while (relations.hasNext()) {
            ...
            /**
             * put edges from ways that are contained by this relation into builder
             */
    ...
    private void waySink(Iterator<Way> ways, MemoryDataSet dataset, Collection<Tag> relationInfo) {
        ...
        while (ways.hasNext()) {
            ...
            if (nodes.size() >= 2) {
                ...
                if (!isReverseOneWayForBicycles(way)) {
                    setEdgesToBuilder(builder, createEdgesFromOsm(nodes, way.getTags(), relationInfo));
                }
                ...
                if (!isOneWayForBicycles(way)) {
                    ...
                    setEdgesToBuilder(builder, createEdgesFromOsm(reverseNodes, way.getTags(), relationInfo));
                }

                // takes all the nodes in the way and puts them into builder together with edges
            } else {
                Iterator<Node> i = nodes.iterator();
                //System.out.println("Found way that has less than two nodes:");
                //System.out.println(way);
                //System.out.println("Has node:");
                while (i.hasNext()) {
                    Node n = i.next();
                    System.out.println(n + " lat:" + n.getLatitude() + " lon:" + n.getLongitude());
                }
            }
        }
    ...
    public List<TEdge> createEdgesFromOsm(ArrayList<Node> nodes, Collection<Tag> wayInfo, Collection<Tag> relationInfo) {
        assert nodes != null;
        assert nodes.size() > 1;
        List<TEdge> edges = new ArrayList<>();
        // takes nodes from the first to last and for each pair an edge is created
        for (int i = 0; i < nodes.size() - 1; i++) {
            Node fromNodeOsm = nodes.get(i);
            Node toNodeOsm = nodes.get(i + 1);
            WayAndRelationalGraphImportTask.TupletEdgeId tupletEdgeId = new WayAndRelationalGraphImportTask.TupletEdgeId(fromNodeOsm.getId(), toNodeOsm.getId());
            ...
            if (edge == null) {
                ...
                edge = createEdge(formNode, toNode, comulativeLength, wayInfo, relationInfo);
            ...
            if (ONEWAY_KEY_PATTERN.matcher(tag.getKey()).matches()) {
                if (ONEWAY_VALUE_PATTERN.matcher(tag.getValue()).matches()) {
                    ...
                    for (Tag tag2 : way.getTags()) {
                        if (NOT_ONEWAY_FOR_CYCLEWAY_KEY_PATTERN.matcher(tag2.getKey()).matches()) {
                            if (NOT_ONEWAY_FOR_CYCLEWAY_VALUE_PATTERN.matcher(tag2.getValue()).matches()) {
                                return false;
                            }
                        }
                    }
                    for (Tag tag3 : way.getTags()) {
                        if (NOT_ONEWAY_FOR_BICYCLE_KEY_PATTERN.matcher(tag3.getKey()).matches()) {
                            if (NOT_ONEWAY_FOR_BICYCLE_VALUE_PATTERN.matcher(tag3.getValue()).matches()) {
                                return false;
                            }
                        }
                    }
    ...
    private boolean isReverseOneWayForBicycles(Way way) {
        ...
        for (Tag tag : way.getTags()) {
            if (ONEWAY_KEY_PATTERN.matcher(tag.getKey()).matches()) {
                if (REVERSE_ONEWAY_VALUE_PATTERN.matcher(tag.getValue()).matches()) {
                    // if it says oneway, check for exception for bicycles
                    for (Tag tag2 : way.getTags()) {
                        if (NOT_ONEWAY_FOR_CYCLEWAY_KEY_PATTERN.matcher(tag2.getKey()).matches()) {
                            if (NOT_ONEWAY_FOR_CYCLEWAY_VALUE_PATTERN.matcher(tag2.getValue()).matches()) {
    ...
    public abstract TEdge createEdge(final TNode fromNode, final TNode toNode,
            final double cumulativeLength, Collection<Tag> wayInfo, Collection<Tag> relationInfo);
